...
- **Whenever we deviate from the plan in `ARCHITECTURE_AND_BUILD_PLAN.md` — changing a module's scope, swapping a tech choice, reordering milestones, adjusting thresholds/routing, or dropping/adding a feature — update that doc in the same turn so it stays an accurate living plan rather than a stale spec.**
...
The full architecture and build plan is in `ARCHITECTURE_AND_BUILD_PLAN.md`. Read it before making significant design decisions — and read Section 0 first, which frames the doc as a living proposal.
