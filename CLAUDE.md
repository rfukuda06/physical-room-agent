...
- **Whenever we change our minds about a decision, hit a non-obvious gotcha, or work around a hard problem, append an entry to `LEARNING.md` in the same turn.** Use the template at the bottom of that file (date, block, tag, what-I-thought, what-actually-happened, lesson). This is the user's study guide for explaining the project later — surprising, corrected, or hard-won knowledge belongs here; boilerplate does not.
