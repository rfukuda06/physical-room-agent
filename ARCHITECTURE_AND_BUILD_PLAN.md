...
| 4 | ✅ Reasoner agent (Beat 2): Claude Sonnet 4.6 integration. `Reasoner` + `ReasonerWorker` in `agents/reasoner.py`. Pydantic `ReasonerOutput` schema with `lamp`/`fan`/`lamp_reason`/`fan_reason` fields. Prompt authorizes lamp and fan actuation; Claude provides judgment. Compounding `session_narrative` in WorldState. ReasonerWorker daemon thread mirrors ObserverWorker pattern. | 2.0 |
| 5 | ✅ Two-beat TTS pipeline: `actuators/speaker.py` — `Speaker` class wraps edge-tts/pyttsx3 with `enqueue_beat1`/`enqueue_beat2` queue. Beat 1 plays immediately from ObserverWorker result; Beat 2 queues after Beat 1 finishes from ReasonerWorker result. | 0.5 |
| 6 | ✅ Decision module: `agents/decisions.py` — `DecisionEngine` translates `ReasonerOutput` into physical actions with 5 code-enforced guardrails (idempotency, cooldown, override lockout, no-person ON, plug-unreachable). `agents/empty_room_watcher.py` — `EmptyRoomWatcher` debounces ≥1→0 person-count transitions into `room_empty_confirmed` synthetic events. Every decision (accepted or refused) published to dashboard broadcaster. | 1.0 |
